...
const ExploreArt = () => {
    ...
    const { isAdmin, isLoggedIn } = useAuth(); // 🆕 Get auth state and role
    ...
    const handleAddNewArt = () => {
        alert("Admin Action: Navigating to Art Management.");
        // In a real application, this would redirect to an Admin form
    };
    ...
    return (
        <div>
            <header
                className="header"
                style={{ background: "#2C2C4E", color: "#F0F0F0", zIndex: 1000 }} // 🎨 Applied dark theme style
            >
            ...
            <div className="explore-art">
                ...
                {/* 🆕 RBAC Implementation: Only show this button to ROLE_ADMIN */}
                {isAdmin && (
                    <button 
                        className="virtual-space-btn" 
                        onClick={handleAddNewArt}
                        style={{ marginBottom: '20px', backgroundColor: '#28a745' }} // Admin button style
                    >
                        + Add New Art
                    </button>
                )}
